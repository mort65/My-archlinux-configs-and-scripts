...
def is_actions_included(windowactions):
    if not windowactions:
        return False
    return (set(ActionIncludeList).issubset(windowactions.split(", ")))


def is_state_excluded(windowstate):
    if not windowstate:
        return False
    return not set(StateExcludeList).isdisjoint(windowstate.split(", "))
...
def get_winlist_props(winlist, props):
    ...
    output = commands.getoutput(inp[:-1]).split("\n\n")
    for out in output:
...
def is_includible(dec_wid, id_include_set, win_type, win_state, win_actions):
    return not (is_state_excluded(win_state) or not is_actions_included(
        win_actions) or dec_wid not in id_include_set and is_type_excluded(win_type))
...
def initialize(id_exclude_set, id_include_set):
    output = commands.getoutput("wmctrl -d;echo;wmctrl -l").split('\n\n')
    ...
    prop_output = get_winlist_props(
        ...
        ("WM_CLASS",
         ...
         "_NET_WM_STATE",
         "_NET_WM_ALLOWED_ACTIONS"))
    for index in xrange(len(win_output)):
        try:
...
def include_win(windowid):
    ...
    if not is_includible(int(windowid, 16), IdIncludeSet,
...
def toggle_maximize_alt(windowid=":ACTIVE:", toggle=-1):
    ...
    if toggle in (-1,
                  0) and (X,
                          Y,
                          Width,
                          Height) == (x - LeftPadding - WinBorder,
