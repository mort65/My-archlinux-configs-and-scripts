...
enum { SchemeNorm, SchemeSel, SchemeUrg, SchemeBtn, SchemeLast }; /* color schemes */
...
static void updatewindowborder(Client *c, int updatesize);
	...
	for(i = 0; i < LENGTH(rules); i++) {
		...
		if (r->class) {
			if ((status = regcomp(&regexcache[i][0], r->class, REG_EXTENDED | REG_NOSUB))) {
				...
				die("Error compiling /%s/: %s\n", r->class, regex_error_buffer);
			}
		}
		if (r->instance) {
...
restack(Monitor *m)
{
	...
	XSync(dpy, False);
	while (XCheckMaskEvent(dpy, EnterWindowMask, &ev));
	if (m == selmon && (m->tagset[m->seltags] & m->sel->tags) && selmon->lt[selmon->sellt] != &layouts[monocle_layout])
		warp(m->sel);
}
...
setlayout(const Arg *arg)
{
	...
	updatewindowborder(selmon->sel, 1);
	arrange(selmon);
	drawbar(selmon);
}
...
void
updatewindowborder(Client *c, int updatesize) {
	if (!c)
		return;
	if (c == mark)
...
togglefloating(const Arg *arg)
{
	...
	updatewindowborder(selmon->sel, 0);
