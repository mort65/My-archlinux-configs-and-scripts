...
enum { SchemeNorm, SchemeSel, SchemeUrg, SchemeBtn, SchemeSym, SchemeLast }; /* color schemes */
...
updatewindowborder(Client *c, int schemeindex, int updatesize)
{
	...
	if (c == mark)
		...
		XSetWindowBorder(dpy, c->win, scheme[i][ColSticky].pixel);
	else if (c->isfloating || !selmon->lt[selmon->sellt]->arrange)
		XSetWindowBorder(dpy, c->win, scheme[i][ColFloat].pixel);
	else {
		XSetWindowBorder(dpy, c->win, scheme[i][ColBorder].pixel);
		if(updatesize)
			resizeclient(c, c->x, c->y, c->w, c->h);
	}
}
