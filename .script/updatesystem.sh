...
if [[ $PacOff == "No" ]]; then
    ...
    echo
    { $SUDO find /etc -regextype posix-extended -regex ".+\.pac(new|save)" 2> /dev/null ; } && { echo ; echo "No pacnew file found" ; } || { echo ; echo -e $red"Error:$reset Pacnew file(s) found! (Use pacdiff to deal with them)" ; read -p "Press enter to exit..." ; exit 1; }
    echo
    echo
fi
if [[ $RSSOff == "No" ]]; then
